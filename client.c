...
int main(int argc, char *argv[]) {
	if (argc < 3 || argc > 4) // Test for correct number of arguments
		...
		printf("<Server Address/Name> <Echo Word> [<Server Port/Service>]");
	char *server = argv[1];
	...
	if (echoStringLen > MAXSTRINGLENGTH) // Check input length
		printf("string too long"); // Third arg (optional): server port/service
		//DieWithUserMessage(echoString, "string too long"); // Third arg (optional): server port/service
	...
	// For the following fields, a zero value means we want "don't care"
...
/*	if (!SockAddrsEqual(servAddr->ai_addr, (struct sockaddr *) &fromAddr))
		...
		printf("recvfrom()", "received a packet from unknown source");
	*/	
